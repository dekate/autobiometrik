...
def test_reload_config_refreshes_runtime_fields_in_place(tmp_path):
    ...
    path.write_text(
        json.dumps({"frista_username": "a", "frista_password": "b", "port": 5000})
    )
    cfg = load_config(path)
    assert cfg.frista_username == "a"
    assert cfg.port == 5000

    # Operator edits the file while the app is running.
    path.write_text(
        json.dumps({"frista_username": "x", "frista_password": "y", "port": 6000})
    )
    returned = reload_config(cfg)

    assert returned is cfg  # mutated in place so live closures see the change
    assert cfg.frista_username == "x"  # credentials reloaded
    assert cfg.frista_password == "y"
...
def test_modern_keys(tmp_path):
    ...
    assert cfg.frista_username == "u"
    assert cfg.frista_password == "p"
    assert cfg.port == 6001
    assert cfg.has_credentials is True


def test_old_credential_keys_are_ignored(tmp_path):
    # Clean break: the pre-rename username/password keys no longer load.
    path = tmp_path / "config.json"
    path.write_text(json.dumps({"username": "u", "password": "p"}))
    cfg = load_config(path)
    assert cfg.frista_username == ""
    assert cfg.frista_password == ""
    assert cfg.has_credentials is False
...
def test_finger_credentials(tmp_path):
    ...
    path.write_text(
        json.dumps(
            {
                "frista_username": "frista-user",
                "frista_password": "frista-pass",
